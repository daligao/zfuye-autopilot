...
def write_from_source(article):
    ...
    prompt = f"""你是一个在小红书和微信公众号都有10万粉的副业博主，擅长把海外信息转化成让中国普通上班族看得懂、用得上的干货文章。
...
根据这篇文章的核心信息，写一篇800-1000字的中文文章。写作风格要求：

【风格】
- 像在跟朋友聊天，用"你"而不是"读者"
- 多用短句，一句话不超过25字
- 禁止翻译腔：不用"以下是""总体来说""值得注意的是""综上所述"
- 用中国人熟悉的例子（微信、淘宝、闲鱼、知乎等）类比说明
- 数字要具体：不说"很多钱"，说"月入2000-5000"

【结构】
1. 开头一句话钩子（用一个反常识的数字或现象）
2. 3-4个核心要点，每个点：先说人话，再给具体方法或数字
3. 结尾：1-2句让读者今天就能做的行动
4. 最后一段自然带出：如果你想搭一个副业网站，Hostinger是我用过最稳的，域名+主机一年不到200块

【格式】
- HTML格式，用<h2><p><ul><li>
- 不要写文章大标题
- 不要```html 代码块标记
- 文末加一行：<p style="color:#999;font-size:13px">资讯来源：{article['source']}</p>"""
        ...
        r = requests.post(
            ...
            json={
                ...
                "messages": [{"role": "user", "content":
                    f"""把这个英文标题改写成中文副业博客标题，要求：
1. 10-18字，用口语，有点标题党但不夸张
2. 优先用这些套路之一：数字（"3个方法""月入5000"）/ 反问（"为什么你还在...？"）/ 对比（"外国人这样做，中国人却..."）
3. 跟副业/赚钱/省钱沾边最好
4. 只输出标题，不加引号、不加序号
